...
const ContactForm = memo (({artisanId}) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        // await apiService.createContactForm(formData);
        setFormStatus({
            submitted: true,
            success: true,
        ...
        <section className=" container d-flex justify-content-center align-items-center p-3"> 
            <div className='artisan-card p-4'>
                ...
                {formStatus.submitted && (
                    <div className='alert alert-success' role='alert' d-flex row>
                ...
                <form className='form-contact d-flex ' onSubmit={handleSubmit}>
                    ...
                    <div className='form-input pt-1'>
                        <div className='form-element d-flex'>
                            ...
                            <input id='objet' name='objet' type='text' recquired className='form-control' value={formData.objet} onChange={handleInputChange}/>
                        </div>
                        <div className='form-element d-flex'>
                            <label label for="nom" className='form-label'>Nom :</label>
                            <input id='nom' name='nom' type='text' recquired className='form-control' value={formData.nom} onChange={handleInputChange}/>
                        </div>
                        <div className='form-element d-flex'>
                            <label label for="email" className='form-label'>Email : </label>
                            <input id='email' name='email' type='email' recquired className='form-control' value={formData.email} onChange={handleInputChange}/>
                        </div>
